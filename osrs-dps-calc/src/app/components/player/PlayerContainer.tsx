...
const PlayerContainer: React.FC = observer(() => {
  ...
  return (
    <div className="flex flex-col w-[350px]">
      ...
      >
        <div className="my-1 flex flex-wrap h-full">
          {loadouts.map((l, ix) => (
            <button
              type="button"
              // eslint-disable-next-line react/no-array-index-key
              key={ix}
              className={`min-w-[40px] text-left first:md:rounded-tl px-3 py-1 border-l-2 first:border-l-0 last:rounded-tr border-body-100 dark:border-dark-300 transition-colors ${selectedLoadout === ix ? 'bg-orange-400 dark:bg-orange-700' : 'bg-btns-400 dark:bg-dark-400'}`}
              onClick={() => {
